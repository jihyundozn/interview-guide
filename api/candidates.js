...
async function deleteCandidateAndAnalyses({ candidateId, analysisId }) {
  let targetCandidateId = candidateId || "";
  let deletedAnalysis = [];
  let deletedCandidate = [];

  // analysis_id만 전달된 경우 먼저 해당 분석의 candidate_id를 찾습니다.
  if (!targetCandidateId && analysisId) {
    ...
    if (analysisRows && analysisRows[0]) {
      targetCandidateId = analysisRows[0].candidate_id;
    }
  }

  // 후보자 id가 있으면 후보자 행을 삭제합니다.
  // ai_resume_analyses.candidate_id는 on delete cascade로 연결되어 있어 분석 결과도 함께 삭제됩니다.
  if (targetCandidateId) {
    deletedCandidate = await supabaseRequest(`candidates?id=eq.${encodeURIComponent(targetCandidateId)}`, {
      method: "DELETE",
    });
  } else if (analysisId) {
    // 후보자 id를 찾지 못한 예외 상황에서는 분석 결과만 삭제합니다.
    deletedAnalysis = await supabaseRequest(`ai_resume_analyses?id=eq.${encodeURIComponent(analysisId)}`, {
      method: "DELETE",
    });
  }
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      if (body.action === "delete") {
        ...
        const deleted = await deleteCandidateAndAnalyses({ candidateId, analysisId });
        return sendJson(res, 200, {
          ok: true,
          ...deleted,
        });
