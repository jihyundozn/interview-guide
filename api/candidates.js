    ...
    if (req.method === "DELETE") {
      ...
      if (candidateId) {
        const remaining = await supabaseRequest(
          `ai_resume_analyses?select=id&candidate_id=eq.${encodeURIComponent(candidateId)}&limit=1`,
          { method: "GET" }
        );

        // 1차 운영에서는 후보자별 분석 결과를 하나의 저장 기록처럼 사용하므로,
        // 분석 결과가 더 없으면 후보자 행도 함께 삭제합니다.
        if (!remaining || remaining.length === 0) {
          deletedCandidate = await supabaseRequest(`candidates?id=eq.${encodeURIComponent(candidateId)}`, {
            method: "DELETE",
          });
        } else if (!analysisId) {
          deletedCandidate = await supabaseRequest(`candidates?id=eq.${encodeURIComponent(candidateId)}`, {
