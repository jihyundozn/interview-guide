...
function naturalizeQuestionAttribution(value) {
  ...
  // AI가 자주 만드는 "OO 경험이 있다고 하셨습니다" 식의 반복적인 첫 문장을
  // 실제 면접관이 자연스럽게 읽을 수 있는 질문으로 후처리합니다.
  // sourceEvidence는 별도 박스에 노출되므로 main에는 출처 확인형 문장을 반복하지 않습니다.
  text = text.replace(/^(.+?)을\s+활용한\s+경험이\s+있다고\s+하셨습니다\.\s*이\s+기술들을\s+사용하여\s+구현한\s+프로젝트에서\s*/g, "$1을 활용한 프로젝트에서 ");
  text = text.replace(/^(.+?)를\s+활용한\s+경험이\s+있다고\s+하셨습니다\.\s*이\s+기술들을\s+사용하여\s+구현한\s+프로젝트에서\s*/g, "$1를 활용한 프로젝트에서 ");
  text = text.replace(/^(.+?)\s+경험이\s+있다고\s+하셨습니다\.\s*이\s+과정에서\s+본인이\s*/g, "$1 과정에서 본인이 ");
  text = text.replace(/^(.+?)\s+경험이\s+있다고\s+하셨습니다\.\s*이\s+과정에서\s*/g, "$1 과정에서 ");
  text = text.replace(/^(.+?)\s+경험이\s+있다고\s+하셨습니다\.\s*/g, "$1 경험과 관련해 ");
  text = text.replace(/^(.+?)\s+경험이\s+있으신\s+것으로\s+확인됩니다\.\s*/g, "$1 경험과 관련해 ");
  text = text.replace(/^(.+?)\s+경험이\s+확인됩니다\.\s*/g, "$1 경험과 관련해 ");
  text = text.replace(/^(.+?)\s+수행하신\s+것으로\s+확인됩니다\.\s*/g, "$1와 관련해 ");
  text = text.replace(/^(.+?)\s+진행하신\s+것으로\s+확인됩니다\.\s*/g, "$1와 관련해 ");
  text = text.replace(/^이\s+기술들을\s+사용하여\s+구현한\s+프로젝트에서\s*/g, "해당 기술을 활용한 프로젝트에서 ");
...
- 특히 main 첫 문장을 매번 "OO 경험이 있다고 하셨습니다.", "OO 경험이 있으신 것으로 확인됩니다.", "이력서에 따르면"으로 시작하지 마세요.
- "이 과정에서 본인이", "이 기술들을 사용하여"처럼 앞 문장에 기대는 표현을 반복하지 말고, 질문 한 문장만 읽어도 자연스럽게 이해되도록 작성하세요.
- 질문은 "짧은 상황 앵커 + 확인하고 싶은 핵심 질문" 구조로 작성하세요. 예: "MSSQL에서 Oracle로 마이그레이션할 때, 가장 신중하게 판단한 설계 포인트와 선택 기준을 설명해 주세요."
- "설명해 주세요"만 반복하지 말고, 질문 목적에 따라 "어떤 기준으로 판단하셨나요?", "가장 중요하게 본 지점은 무엇이었나요?", "어디까지 직접 맡으셨나요?", "결과를 어떻게 확인하셨나요?"처럼 문장 구조를 다양화하세요.
...
- 나쁜 질문 예시: "MSSQL에서 Oracle로 마이그레이션을 주도한 경험이 있다고 하셨습니다. 이 과정에서 본인이 판단한 주요 설계 선택과 그 이유를 설명해 주세요."
- 좋은 질문 예시: "MSSQL에서 Oracle로 마이그레이션할 때, 가장 신중하게 판단한 설계 포인트와 선택 기준을 설명해 주세요."
- 나쁜 질문 예시: "Spring Data JPA와 QueryDSL을 활용한 경험이 있다고 하셨습니다. 이 기술들을 사용하여 구현한 프로젝트에서 본인이 직접 설정한 성과 산정 기준을 말씀해 주세요."
- 좋은 질문 예시: "Spring Data JPA와 QueryDSL을 활용한 프로젝트에서, 조회 성능이나 개발 생산성 측면에서 기대한 개선 효과와 실제 확인 기준을 말씀해 주세요."
...
function shouldRunLanguagePolish(analysis) {
  ...
  return /(이선우지원자|지원자는\d|[가-힣]{2,4}지원자|RESTAPI|OCRAPI|백엔드개발|데이터베이스모델링|요구사항분석|Java와Spring|TypeScript와Next|Next\.\s+js|CI\/CD환경|경험이풍부|경험이있|경험이\s*있다고\s*하셨습니다|경험이\s*있으신\s*것으로\s*확인됩니다|이\s*기술들을\s*사용하여|보유하고있|추가검증|추가확인|설명해주세요)/.test(text);
}
...
- 질문 main이 "OO 경험이 있다고 하셨습니다.", "OO 경험이 있으신 것으로 확인됩니다.", "이력서에 따르면"으로 시작하지 않게 자연스럽게 고치세요.
- "이 과정에서 본인이", "이 기술들을 사용하여"처럼 앞 문장에 기대는 표현을 반복하지 말고, 질문 한 문장만 읽어도 이해되도록 고치세요.\n- 질문 의도는 "~확인하기 위한 질문입니다." 형태로 통일하세요.\n\nJSON:\n${JSON.stringify(normalized)}` }] }],
        temperature: 0,
...
- "OO 경험이 있다고 하셨습니다.", "OO 경험이 있으신 것으로 확인됩니다.", "이력서에 따르면"으로 main을 시작하지 마세요.
- "이 과정에서 본인이", "이 기술들을 사용하여"처럼 앞 문장에 기대는 표현을 반복하지 말고, 질문 한 문장만 읽어도 자연스럽게 이해되도록 작성하세요.
- 질문은 "짧은 상황 앵커 + 확인하고 싶은 핵심 질문" 구조로 작성하세요.
