...
function buildPolishPrompt(analysis) {
  return `아래 JSON은 채용 이력서 분석 결과입니다. 내용의 판단 방향은 유지하되, 모든 문장을 기존 표현에 얽매이지 말고 사람이 읽기 좋은 자연스러운 한국어 문장으로 완전히 다시 써서 동일한 JSON 구조로만 반환하세요.

수정 규칙:
- 띄어쓰기가 무너진 표현을 반드시 자연스럽게 복원하세요. 예: "백엔드개발경험" → "백엔드 개발 경험", "외부시스템연동" → "외부 시스템 연동", "후보자는Java/Spring기반" → "후보자는 Java/Spring 기반".
- 이력서 원문 제목이나 JD 문구를 그대로 복사한 표현은 평가 언어로 바꾸세요.
- summary 본문에 점수 표현은 넣지 마세요. 점수는 overallFitScore 필드에만 유지하세요.
- 의미를 과장하거나 새 사실을 추가하지 마세요.
- 12자 이상 붙어 있는 한국어 표현이 나오지 않도록 조사와 명사 사이를 자연스럽게 띄어 쓰세요.
- 모든 값은 한국어 자연문으로 작성하세요.
- JSON 외 설명 문장은 절대 쓰지 마세요.
...
async function polishAnalysisIfNeeded({ analysis, model, apiKey }) {
  if (process.env.DISABLE_ANALYSIS_POLISH === 'true') return analysis;
  // PDF 추출 문장이 조금이라도 붙어 있으면 면접관 사용성이 크게 떨어지므로,
  // 실제 운영에서는 항상 2차 문장 교정 단계를 수행합니다.
  ...
  const response = await fetch('https://api.openai.com/v1/responses', {
    ...
    body: JSON.stringify({
      ...
      input: [
        {
          ...
          content: [
            {
              type: 'input_text',
              text: '당신은 한국어 문장 교정자입니다. 응답은 반드시 요청한 JSON 구조만 반환하세요.',
            },
          ],
        },
        {
          ...
          content: [
            {
              type: 'input_text',
              text: buildPolishPrompt(analysis),
            },
          ],
        },
      ],
      text: {
        format: {
          type: 'json_schema',
          name: 'resume_interview_analysis_polished',
          strict: true,
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const analysis = normalizeAnalysisPayload(calibrateScores(polishedAnalysis));
