...
function restoreKoreanSpacing(value) {
  const phraseRules = [
    ...
    ["관련경험", "관련 경험"],
    ["이상의백엔드", "이상의 백엔드"],
    ["CI/CD환경구축경험", "CI/CD 환경 구축 경험"],
    ["CI/CD환경구축", "CI/CD 환경 구축"],
    ["CI/CD환경", "CI/CD 환경"],
    ["SpringBoot를활용한", "SpringBoot를 활용한"],
    ["SpringBoot를활용", "SpringBoot를 활용"],
    ["Java와SpringBoot", "Java와 SpringBoot"],
    ["TypeScript와Next.js", "TypeScript와 Next.js"],
    ["TypeScript와Next§DOT§js", "TypeScript와 Next§DOT§js"],
    ["경험은확인", "경험은 확인"],
    ["확인이필요", "확인이 필요"],
    ["보유하고있", "보유하고 있"],
    ["갖추고있", "갖추고 있"],
  ...
  // 기술명 내부의 마침표는 문장부호로 처리하지 않도록 보호합니다.
  text = text
    .replace(/Next\s*\.\s*js/gi, "Next§DOT§js")
    .replace(/Node\s*\.\s*js/gi, "Node§DOT§js")
    .replace(/Vue\s*\.\s*js/gi, "Vue§DOT§js");
  phraseRules.forEach(([from, to]) => {
    text = text.replace(new RegExp(from, "g"), to);
  });

  return text
    .replace(/\b(Java|SpringBoot|Spring|TypeScript|React|Python|Jenkins|GitLab|CI\/CD|REST API|OCR API|Next§DOT§js|Node§DOT§js|Vue§DOT§js)(와|과|을|를|은|는|이|가|로|으로)(?=[A-Za-z0-9가-힣])/g, "$1$2 ")
    .replace(/\b(Java|SpringBoot|Spring|TypeScript|React|Python|Jenkins|GitLab|CI\/CD|REST API|OCR API|Next§DOT§js|Node§DOT§js|Vue§DOT§js)(?=(관련|기반|경험|설계|구축|활용|개발|운영|환경|시스템))/g, "$1 ")
    .replace(/([가-힣]{2,4})지원자/g, "$1 지원자")
    .replace(/지원자는(?=[0-9A-Za-z가-힣])/g, "지원자는 ")
    .replace(/(\d+)년(\d+)개월/g, "$1년 $2개월")
    .replace(/(\d+년)\s*이상/g, "$1 이상")
    .replace(/(\d+개월)\s*이상/g, "$1 이상")
    .replace(/기반의(?=[가-힣A-Za-z0-9])/g, "기반의 ")
    .replace(/(지원자|경험|경력|요건|능력|강점|문제|역할|프로젝트|시스템|환경|설계|모델링|구축|포지션|과정|기준|성과)(은|는|이|가|을|를|에|에서|의|와|과)(?=[0-9A-Za-z가-힣])/g, "$1$2 ")
    .replace(/경험이풍부/g, "경험이 풍부")
    .replace(/경험이있/g, "경험이 있")
    .replace(/경험도있/g, "경험도 있")
    ...
    .replace(/프로젝트에\s*서/g, "프로젝트에서")
    .replace(/서가장/g, "서 가장")
    .replace(/가장도전적/g, "가장 도전적")
    .replace(/도전적이었던부분/g, "도전적이었던 부분")
    .replace(/부분은무엇/g, "부분은 무엇")
    .replace(/설명해주세요/g, "설명해 주세요")
    .replace(/말씀해주세요/g, "말씀해 주세요")
    .replace(/확인해주세요/g, "확인해 주세요")
...
function isGenericInterviewQuestion(text) {
  ...
  const hasSpecificAnchor = /(프로젝트|시스템|서비스|모듈|연동|전환|개선|구축|설계|모델링|정산|OCR|REST API|CI\/CD|GitLab|Jenkins|CRM\/BI)/.test(t);
  const asksForConcreteProof = /(본인|직접|역할|기여|판단\s*기준|선택\s*이유|산정\s*기준|성과\s*근거|설계\s*방식|처리\s*방식|트레이드오프|의사결정)/.test(t);
  return genericPatterns.some((pattern) => pattern.test(t)) && !(hasSpecificAnchor && asksForConcreteProof);
}

function buildSpecificQuestionFromEvidence(item) {
  const evidence = cleanText(item.sourceEvidence || item.evidence || item.relatedCompetency || "이력서에 기재된 관련 경험");
  const competency = cleanText(item.relatedCompetency || item.competency || "직무 연관성");
  if (/OCR|외부 시스템|연동/.test(evidence + competency)) {
    return `${evidence}와 관련해 외부 API 연동 방식, 후처리 로직 설계, 오류 대응 과정에서 본인이 직접 판단한 기준과 기여 범위를 구체적으로 설명해 주세요.`;
  }
  if (/CI\/CD|Jenkins|GitLab|배포|구축/.test(evidence + competency)) {
    return `${evidence}와 관련해 배포 환경을 구성할 때 본인이 맡은 역할, 적용 방식, 운영 안정성에 미친 영향을 구체적으로 설명해 주세요.`;
  }
  if (/REST API|API|데이터베이스|모델링|Spring|Java/.test(evidence + competency)) {
    return `${evidence}와 관련해 API 설계 또는 데이터 모델링 과정에서 본인이 직접 결정한 설계 기준과 성능·운영 측면의 고려사항을 구체적으로 설명해 주세요.`;
  }
  return `${evidence}와 관련해 본인이 직접 수행한 역할, 판단 기준, 실행 과정, 결과 근거를 구체적으로 설명해 주세요.`;
}

function normalizeQuestion(item, index) {
  // 질문 문장은 모델이 생성한 문장을 우선 보존합니다.
  // 이전 버전처럼 코드 템플릿으로 질문을 다시 만들면 질문이 일반화되고 띄어쓰기가 깨질 수 있어 제거했습니다.
...
- "가장 도전적이었던 부분은 무엇이었나요?", "어떤 어려움이 있었나요?", "어떻게 해결했나요?"처럼 어떤 후보자에게나 물을 수 있는 질문은 금지합니다.
- 모든 질문은 반드시 이력서의 특정 프로젝트명/업무명/기술 경험을 포함하고, 본인 역할·판단 기준·설계 선택·성과 근거 중 하나 이상을 확인해야 합니다.
...
function shouldRunLanguagePolish(analysis) {
  ...
  return /(이선우지원자|지원자는\d|[가-힣]{2,4}지원자|RESTAPI|OCRAPI|백엔드개발|데이터베이스모델링|요구사항분석|Java\s*와Spring|TypeScript\s*와Next|Next\.\s+js|CI\/CD환경|경험이풍부|경험이있|보유하고있|추가검증|추가확인|설명해주세요|가장도전적|무엇이었나요)/.test(text);
}
    ...
    const response = await fetch("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({
        ...
        input: [{
          ...
          content: [{
            type: "input_text",
            text: `아래 JSON은 이력서 분석 결과입니다. 점수와 판단 내용은 바꾸지 말고, 모든 한국어 문장을 면접관이 읽을 수 있는 자연스러운 문장으로 다시 작성해 주세요. 단순 맞춤법 교정이 아니라, 붙어 있는 단어와 어색한 질문을 자연스럽게 재작성해야 합니다.
...
- "가장 도전적이었던 부분은 무엇인가요?", "어떤 어려움이 있었나요?" 같은 일반 질문은 금지하고, sourceEvidence의 특정 경험을 바탕으로 본인 역할·판단 기준·성과 근거를 묻는 질문으로 바꾸세요.
- 질문 main에는 sourceEvidence의 핵심 경험이나 기술 맥락을 자연스럽게 포함하세요.
...
function formatErrorMessage(error, fallback = "AI 분석 중 오류가 발생했습니다.") {
...
function jsonResponse(res, statusCode, payload) {
  ...
  if (safePayload.error && typeof safePayload.error !== "string") {
    safePayload.error = formatErrorMessage(safePayload.error);
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const polished = await polishKoreanWithOpenAI(raw);
    const analysis = normalizeAnalysis(polished);
    validateAnalysis(analysis);

    const responsePayload = { analysis, engine: "openai-pdf", model: OPENAI_ANALYSIS_MODEL };
    if (body.debug === true || process.env.AI_RESUME_DEBUG === "true") {
    ...
    return jsonResponse(res, 500, { error: formatErrorMessage(error) });
  }
}
