...
async function polishAnalysisIfNeeded({ analysis, model, apiKey }) {
  ...
  try {
    const polished = normalizeAnalysisPayload(parseAnalysisJson(outputText));
    // 1차 교정 후에도 질문/의도 문장에 띄어쓰기나 어체 문제가 남는 경우가 있어,
    // 최종 표시 전 문장 검수 단계를 한 번 더 수행합니다.
    return await rewriteAnalysisTextOnly({ analysis: polished, model, apiKey });
  } catch (error) {
