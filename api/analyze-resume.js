...
${hasPdf ? `[지원자 이력서]
PDF 원본 파일이 함께 첨부되어 있습니다. PDF 원본을 직접 읽어 분석하세요.
텍스트 레이어의 띄어쓰기가 깨져 있더라도, 출력은 반드시 자연스러운 한국어 띄어쓰기로 작성하세요.` : `[지원자 이력서 텍스트]\n${String(resumeText || "").slice(0, 30000)}`}`;
}
...
async function callGemini({ prompt, resumePdfBase64, resumePdfImages, resumePdfMimeType }) {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) throw new Error("GEMINI_API_KEY 환경변수가 설정되어 있지 않습니다.");

  const parts = [{ text: prompt }];

  // 이미지 배열이 있으면 페이지별 이미지로 전달 (텍스트 레이어 무시, 시각적으로 읽음)
...
function getPolishPrompt() {
  return `당신은 한국어 채용 문서 전문 에디터입니다.
아래 JSON은 이미 Gemini가 PDF 이력서를 읽고 분석한 결과입니다.

당신의 역할은 "재분석"이 아니라 "최종 한국어 문장 정리"입니다.
판단, 점수, 항목 수, 질문 수, 후보자 평가 방향은 바꾸지 마세요.
오직 사람이 읽기 좋은 한국어로 띄어쓰기, 어체, 항목명, 문장 끝맺음만 다듬으세요.

반드시 지켜야 할 규칙:
1. 숫자 점수는 절대 변경하지 마세요.
2. jdFit 항목 수와 customQuestions 항목 수를 변경하지 마세요.
3. 각 항목의 의미와 판단 강도는 유지하세요.
4. 붙어 있는 한국어 복합어는 자연스럽게 띄어쓰세요.
   - 백엔드개발경험 → 백엔드 개발 경험
   - 외부시스템연동 → 외부 시스템 연동
   - 요구사항분석및업무계획수립능력 → 요구사항 분석 및 업무 계획 수립 능력
   - TypeScript, Next.js 기반서비스개발경험 → TypeScript/Next.js 기반 서비스 개발 경험
5. jdFit.competency는 면접관이 읽기 좋은 짧은 평가 항목명으로 정리하세요.
   예: "Java/Spring 기반백엔드개발경험" → "Java/Spring 기반 백엔드 개발 경험"
6. 질문 main은 면접관이 그대로 읽을 수 있는 존댓말 문장으로 정리하세요.
   문장 끝은 "설명해 주세요.", "말씀해 주세요.", "구체적으로 설명해 주세요." 형태로 통일하세요.
7. intent는 반드시 "~을 확인하기 위한 질문입니다." 또는 "~을 평가하기 위한 질문입니다." 형태의 완성문으로 작성하세요.
   "~하기 위함", "~하기 위해"로 끝내지 마세요.
8. 원문 인용, 파일명, 페이지 표기, OCR 깨짐 표현을 새로 만들거나 남기지 마세요.
9. 반환은 설명 없이 JSON만 반환하세요.`;
}

function getPolishUserPayload(analysis) {
  return JSON.stringify({ analysis }, null, 2);
}

async function callOpenAIPolish(analysis) {
  ...
  try {
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: getPolishPrompt() },
          { role: "user", content: getPolishUserPayload(analysis) },
        ],
      }),
    });
    ...
    const data = await response.json().catch(() => ({}));
    if (!response.ok) {
      console.warn("OpenAI polish skipped:", data?.error?.message || response.status);
      return analysis;
    }
    ...
    const candidate = parsed.analysis || parsed;
    return mergePolishedAnalysis(analysis, candidate);
  } catch (error) {
    console.warn("OpenAI polish failed, using Gemini result:", error?.message || error);
    return analysis;
  } finally {
    clearTimeout(timeout);
  }
...
function mergePolishedAnalysis(original, polishedRaw) {
  const polished = normalizeAnalysis(polishedRaw);

  // 구조가 무너지면 원본 Gemini 결과를 사용한다.
  if (!polished.summary || polished.summary.length < 20) return original;
  if (!Array.isArray(polished.jdFit) || polished.jdFit.length !== original.jdFit.length) return original;
  if (!Array.isArray(polished.customQuestions) || polished.customQuestions.length !== original.customQuestions.length) return original;
  if (!Array.isArray(polished.verificationPoints) || polished.verificationPoints.length < 3) return original;

  return {
    ...original,
    summary: polished.summary || original.summary,
    verificationPoints: polished.verificationPoints.length ? polished.verificationPoints : original.verificationPoints,
    jdFit: original.jdFit.map((item, index) => {
      const next = polished.jdFit[index] || {};
      return {
        ...item,
        competency: next.competency || item.competency,
        evidence: next.evidence || item.evidence,
        // 점수와 id는 Gemini의 원본 판단을 유지한다.
        score: item.score,
        id: item.id,
      };
    }),
    customQuestions: original.customQuestions.map((item, index) => {
      const next = polished.customQuestions[index] || {};
      return {
        ...item,
        type: next.type || item.type,
        main: next.main || item.main,
        follow: Array.isArray(next.follow) && next.follow.length ? next.follow : item.follow,
        intent: next.intent || item.intent,
        relatedCompetency: next.relatedCompetency || item.relatedCompetency,
        sourceEvidence: next.sourceEvidence || item.sourceEvidence,
        points: Array.isArray(next.points) && next.points.length ? next.points : item.points,
        id: item.id,
      };
    }),
    overallFitScore: original.overallFitScore,
  };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const raw = await callGemini({ prompt, resumePdfBase64, resumePdfImages, resumePdfMimeType });
    let analysis = normalizeAnalysis(raw);
    validateAnalysis(analysis);

    // Gemini는 PDF 이해를 담당하고, OpenAI는 한국어 띄어쓰기/어체/항목명 최종 정리를 담당한다.
    analysis = await callOpenAIPolish(analysis);
    validateAnalysis(analysis);

    return jsonResponse(res, 200, {
      analysis,
      engine: "gemini-pdf+openai-polish",
      model: GEMINI_MODEL,
      polishModel: process.env.OPENAI_API_KEY ? OPENAI_POLISH_MODEL : "disabled",
    });
  } catch (error) {
