...
function normalizeAnalysis(raw) {
  const source = raw || {};
  const jdFit = toArray(source.jdFit || source.jdCompetencyFit || source.competencyFit).map((item, index) => ({
  ...
  const customQuestions = toArray(source.customQuestions || source.recommendedQuestions || source.questions)
  ...
  let overallFitScore = clampScore(source.overallFitScore || source.fitScore || source.score, 0);
  if (jdFit.length) {
    const avg = jdFit.reduce((sum, item) => sum + item.score, 0) / jdFit.length;
    const lowCount = jdFit.filter((item) => item.score < 70).length;
    overallFitScore = Math.round(avg);
...
- 커뮤니케이션/팀워크/일정관리처럼 이력서에서 간접적으로만 확인되는 항목은 구체적 협업 상황과 본인 역할이 없으면 75점 이상을 주지 마세요.
...
[질문 작성 규칙]
- 모든 질문은 이력서의 특정 프로젝트명/업무명/기술 경험을 포함하세요.
- 질문은 본인 역할, 판단 기준, 설계 선택, 성과 근거, 문제 해결 과정 중 하나 이상을 구체적으로 묻도록 작성하세요.
- "가장 도전적이었던 부분은 무엇인가요?", "어떤 어려움이 있었나요?", "어떻게 해결했나요?"처럼 일반 질문으로 끝내지 마세요.
