...
function normalizeQuestion(item, index) {
  const follow = toArray(item?.follow || item?.followUps || item?.followup).map(cleanText).filter(Boolean).slice(0, 3);
  const rawMain = cleanText(item?.main || item?.question || "");
  const main = toCompleteQuestionMain(item, rawMain, follow);
...
- 후보자가 이력서에 실제로 작성한 내용은 "이력서에 ~라고 적어주셨는데", "이력서에서 ~ 경험을 확인했는데", "~ 프로젝트에서 ~를 수행하신 것으로 보이는데"처럼 자연스러운 면접 질문 어투로 표현하세요.
- AI가 추론한 내용은 "이력서상 ~은 명확히 확인되지 않습니다", "~에 대한 추가 확인이 필요합니다", "관련 경험이 있다면 설명해 주세요"처럼 표현하세요.
...
- 후보자가 이력서에 실제로 작성한 내용은 "이력서에 ~라고 적어주셨는데", "이력서에서 ~ 경험을 확인했는데", "~ 프로젝트에서 ~를 수행하신 것으로 보이는데"처럼 자연스러운 면접 질문 어투로 표현하세요.
- 이력서에서 직접 확인되지 않는 내용은 "이력서상 ~은 명확히 확인되지 않습니다", "관련 경험이 있다면 설명해 주세요"처럼 표현하세요.
