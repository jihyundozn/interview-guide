...
export default async function handler(req) {
  ...
  try {
    ...
    if (!positionContext) return jsonResponse(400, { error: "포지션 분석 기준이 필요합니다." });
    if (!resumePdfBase64 && !String(resumeText || "").trim()) {
      return jsonResponse(400, { error: "이력서 PDF 또는 이력서 텍스트가 필요합니다." });
    }
    if (resumePdfBase64 && String(resumePdfBase64).length > 10_500_000) {
      return jsonResponse(413, { error: "PDF 파일이 너무 큽니다. 7MB 이하 파일로 다시 업로드해 주세요." });
    }
    ...
    return jsonResponse(200, { analysis, engine: "gemini-pdf", model: GEMINI_MODEL });
  } catch (error) {
    console.error(error);
    return jsonResponse(500, { error: error.message || "AI 분석 중 오류가 발생했습니다." });
  }
}
