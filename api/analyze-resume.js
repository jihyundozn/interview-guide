...
function restoreKoreanSpacing(value) {
  const phraseRules = [
    ...
    ["다양한프로젝트", "다양한 프로젝트"],
    ["팀리더", "팀 리더"],
    ["커뮤니케이션과협업능력", "커뮤니케이션과 협업 능력"],
    ["협업능력", "협업 능력"],
    ["추가검증", "추가 검증"],
    ["추가확인", "추가 확인"],
    ["선호자격", "선호 자격"],
    ["가나안금융정보추출시스템", "가나안 금융 정보 추출 시스템"],
    ["금융정보추출시스템", "금융 정보 추출 시스템"],
    ["구축프로젝트", "구축 프로젝트"],
    ["연동시", "연동 시"],
    ["맡은역할", "맡은 역할"],
    ["기반개발경험", "기반 개발 경험"],
  ...
  return text
    ...
    .replace(/([가-힣]{2,4})지원자(?=(는|가|의|를|에게|는))/g, "$1 지원자")
    .replace(/지원자(?=(는|가|의|를|에게))/g, "지원자")
    .replace(/(\d+\s*년)\s*(\d+\s*개월)의(?=[가-힣A-Za-z])/g, "$1 $2의 ")
    .replace(/(\d+\s*개월)의(?=[가-힣A-Za-z])/g, "$1의 ")
    .replace(/기반의(?=[가-힣A-Za-z])/g, "기반의 ")
    .replace(/설계와(?=[가-힣A-Za-z])/g, "설계와 ")
    .replace(/모델링에(?=[가-힣A-Za-z])/g, "모델링에 ")
    .replace(/경험이(?=[가-힣A-Za-z])/g, "경험이 ")
    .replace(/경험은(?=[가-힣A-Za-z])/g, "경험은 ")
    .replace(/경험을(?=[가-힣A-Za-z])/g, "경험을 ")
    .replace(/능력을(?=[가-힣A-Za-z])/g, "능력을 ")
    .replace(/역할을(?=[가-힣A-Za-z])/g, "역할을 ")
    .replace(/강점을(?=[가-힣A-Za-z])/g, "강점을 ")
    .replace(/부합합니다/g, "부합합니다")
    ...
    .replace(/SpringBoot\s+를/g, "SpringBoot를")
...
function hardPolishKorean(value) {
  let text = cleanText(value);
  const fixes = [
    [/([가-힣]{2,4})지원자/g, "$1 지원자"],
    [/지원자는(?=\d)/g, "지원자는 "],
    [/(\d+)년\s*(\d+)개월의(?=\S)/g, "$1년 $2개월의 "],
    [/의백엔드/g, "의 백엔드"],
    [/의프론트엔드/g, "의 프론트엔드"],
    [/의다양한/g, "의 다양한"],
    [/기반의(?=\S)/g, "기반의 "],
    ...
    [/확인해주세요/g, "확인해 주세요"],
    [/Java\s+와/g, "Java와"],
    [/TypeScript\s+와/g, "TypeScript와"],
    [/Next\.\s*js/g, "Next.js"],
    [/REST\s*API/g, "REST API"],
    [/OCR\s*API/g, "OCR API"],
    [/([가-힣])REST API/g, "$1 REST API"],
    [/([가-힣])OCR API/g, "$1 OCR API"],
    [/OCR API연동/g, "OCR API 연동"],
    [/REST API설계/g, "REST API 설계"],
  ];
  fixes.forEach(([from, to]) => { text = text.replace(from, to); });
  return text.replace(/\s{2,}/g, " ").trim();
}

function deepPolishAnalysisText(analysis) {
  const cloned = JSON.parse(JSON.stringify(analysis || {}));
  cloned.summary = hardPolishKorean(cloned.summary || "");
  cloned.jdFit = toArray(cloned.jdFit).map((item) => ({
    ...item,
    competency: hardPolishKorean(item.competency || ""),
    evidence: hardPolishKorean(item.evidence || ""),
  }));
  cloned.verificationPoints = toArray(cloned.verificationPoints).map(hardPolishKorean).filter(Boolean);
  cloned.customQuestions = toArray(cloned.customQuestions).map((q) => ({
    ...q,
    type: hardPolishKorean(q.type || ""),
    main: hardPolishKorean(q.main || ""),
    follow: toArray(q.follow).map(hardPolishKorean).filter(Boolean),
    intent: hardPolishKorean(q.intent || ""),
    relatedCompetency: hardPolishKorean(q.relatedCompetency || ""),
    sourceEvidence: hardPolishKorean(q.sourceEvidence || ""),
    points: toArray(q.points).map(hardPolishKorean).filter(Boolean),
  }));
  return cloned;
}

function hasSevereSpacingIssueInAnalysis(analysis) {
  const text = JSON.stringify(analysis || "");
  return /[가-힣]{9,}/.test(text) || /(지원자는\d|의백엔드|기반의[가-힣]|개발경력|개발경험|연동시|맡은역할|설명해주세요|Next\.\s*js)/.test(text);
}
...
async function callOpenAITextPolish(analysis) {
  const apiKey = process.env.OPENAI_API_KEY;
  ...
  try {
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "당신은 한국어 채용 문서 교정 담당자입니다. JSON 구조와 판단 내용, 점수는 유지하고, 모든 한국어 문장만 자연스러운 표준 한국어 띄어쓰기와 어체로 교정합니다. 붙어 있는 명사구를 반드시 사람이 읽기 좋게 띄어 씁니다.",
          },
          {
            role: "user",
            content: `아래 이력서 분석 JSON을 교정하세요.\n\n규칙:\n- overallFitScore와 jdFit[].score 숫자는 절대 변경하지 마세요.\n- 배열 개수와 필드명은 유지하세요.\n- 원문을 추가하지 말고, 기존 의미만 자연스럽게 다듬으세요.\n- \"이선우지원자는3년4개월의백엔드개발경력\" 같은 붙은 문장은 \"이선우 지원자는 3년 4개월의 백엔드 개발 경력\"처럼 띄어 쓰세요.\n- \"Java 와\"는 \"Java와\"로, \"Next. js\"는 \"Next.js\"로, \"OCRAPI\"는 \"OCR API\"로 고치세요.\n- 질문은 반드시 \"~설명해 주세요.\"처럼 면접관이 그대로 읽을 수 있게 작성하세요.\n\nJSON:\n${payload}`,
          },
        ],
        temperature: 0,
        max_tokens: 4500,
    ...
    clearTimeout(timeout);
    const data = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(data?.error?.message || "OpenAI 문장 교정 실패");
    const text = extractOpenAiText(data);
    const polishedRaw = parseJsonLoose(text);
    const polished = normalizeAnalysis(polishedRaw);

    // 점수는 원본 분석 기준을 보존한다.
    polished.overallFitScore = analysis.overallFitScore;
    polished.jdFit = polished.jdFit.map((item, index) => ({
      ...item,
      id: analysis.jdFit?.[index]?.id || item.id,
      score: analysis.jdFit?.[index]?.score ?? item.score,
    }));
    return deepPolishAnalysisText(polished);
  } catch (error) {
    clearTimeout(timeout);
    console.warn("OpenAI text polish skipped:", error?.message || error);
    return analysis;
  }
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    let analysis = deepPolishAnalysisText(normalizeAnalysis(raw));

    // GPT-4o가 분석했더라도 한국어 명사구를 라벨처럼 붙여 쓰는 경우가 있어,
    // 문제가 감지되면 텍스트 전용 교정 단계를 한 번 더 수행한다.
    if (hasSevereSpacingIssueInAnalysis(analysis)) {
      analysis = await callOpenAITextPolish(analysis);
    }

    analysis = deepPolishAnalysisText(analysis);
    validateAnalysis(analysis);

    return jsonResponse(res, 200, { analysis, engine: "openai-vision-polished", model: OPENAI_ANALYSIS_MODEL });
  } catch (error) {
