...
function normalizeGeneratedText(value) {
  ...
  return text
    ...
    .replace(/(?<!\s)(다만|면접에서는|해당 경험은|이 과정에서|특히|반면|따라서|추가로)(?=\S)/g, ' $1')
    .replace(/\s+/g, ' ')
    .trim();
}
...
function buildPolishPrompt(analysis) {
  return `아래 JSON은 채용 이력서 분석 결과입니다. 내용의 판단 방향은 유지하되, 모든 한국어 문장을 사람이 읽기 좋은 자연스러운 문장으로 다시 다듬어 동일한 JSON 구조로만 반환하세요.

수정 규칙:
- 띄어쓰기가 무너진 표현을 반드시 자연스럽게 복원하세요. 예: "백엔드개발경험" → "백엔드 개발 경험", "외부시스템연동" → "외부 시스템 연동".
...
async function polishAnalysisIfNeeded({ analysis, model, apiKey }) {
  if (!hasSevereSpacingIssue(analysis)) return analysis;
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const analysis = await polishAnalysisIfNeeded({
