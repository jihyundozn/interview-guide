...
function restoreKoreanSpacing(value) {
  return String(value || "")
    ...
    .replace(/,(?=\S)/g, ", ")
    .replace(/\.(?=\S)/g, ". ")
    .replace(/([가-힣])(기반|경력|개발자|개발|운영|관리|설계|구축|개선|전환|연동|분석|검증|확인|경험|프로젝트|성과|역량|질문입니다)/g, "$1 $2")
...
PDF 원본의 텍스트 레이어에서 띄어쓰기가 깨져 있으면 이 재구성 텍스트를 우선 참고하고, PDF 원본은 누락 내용 확인용으로 사용하세요.` : `[지원자 이력서 텍스트]\n${String(resumeText || "").slice(0, 30000)}`}`;
}
