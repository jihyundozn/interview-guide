const OPENAI_ANALYSIS_MODEL = process.env.OPENAI_ANALYSIS_MODEL || process.env.OPENAI_MODEL || "gpt-4o";
...
function normalizeAnalysis(raw) {
  ...
  if (jdFit.length) {
    // 상단 AI 참고 점수는 JD 역량 적합도 평균값을 기준으로 산정한다.
    const avg = jdFit.reduce((sum, item) => sum + item.score, 0) / jdFit.length;
    overallFitScore = clampScore(avg, overallFitScore);
  }
...
${hasPdf ? `[지원자 이력서]
PDF를 페이지별 이미지로 변환한 파일이 함께 첨부되어 있습니다. 이미지를 직접 읽어 이력서 내용을 이해한 뒤 분석하세요.
이미지 속 원문이나 기술 라벨이 붙어 보이더라도, 출력은 반드시 자연스러운 한국어 띄어쓰기와 면접관용 문장으로 새로 작성하세요.` : `[지원자 이력서 텍스트]\n${String(resumeText || "").slice(0, 30000)}`}`;
}
...
function buildOpenAIUserContent({ prompt, resumePdfBase64, resumePdfImages, resumePdfMimeType }) {
  const content = [{ type: "text", text: prompt }];

  if (Array.isArray(resumePdfImages) && resumePdfImages.length > 0) {
    resumePdfImages.slice(0, 6).forEach((imgBase64) => {
      content.push({
        type: "image_url",
        image_url: {
  ...
  // 현재 ai-resume.html은 PDF를 페이지별 이미지로 변환해 resumePdfImages로 보내는 구조다.
  // 이미지 변환이 실패한 경우에는 텍스트 입력만으로 분석하되, 텍스트도 없으면 사용자에게 다시 업로드를 요청한다.
  if (resumePdfBase64 && resumePdfMimeType && !String(resumePdfMimeType).includes("pdf")) {
    content.push({
      type: "image_url",
      image_url: {
        url: `data:${resumePdfMimeType};base64,${resumePdfBase64}`,
        detail: "high",
      },
    });
  }
...
async function callOpenAIAnalysis({ prompt, resumePdfBase64, resumePdfImages, resumePdfMimeType }) {
  ...
  const hasImageFallback = resumePdfBase64 && resumePdfMimeType && !String(resumePdfMimeType).includes("pdf");

  if (!hasImages && !hasImageFallback) {
    // PDF 원본을 OpenAI에 직접 보내는 대신, 안정성을 위해 브라우저에서 변환된 페이지 이미지를 사용한다.
    // 이 오류가 발생하면 PDF 이미지 변환이 실패한 것이므로 ai-resume.html의 PDF 업로드를 다시 시도하면 된다.
    throw new Error("PDF 이미지를 분석할 수 없습니다. PDF를 다시 업로드하거나 용량을 줄여 주세요.");
  }
    ...
    body: JSON.stringify({
      ...
      response_format: {
        ...
        json_schema: {
          ...
          strict: true,
          schema: getOpenAIJsonSchema(),
        },
      },
      messages: [
        {
          role: "system",
          content: "당신은 한국어 문장 품질이 매우 중요한 채용 면접 설계 전문가입니다. PDF 이미지를 읽고, 원문을 복사하지 말고, 면접관이 바로 읽을 수 있는 자연스러운 한국어 분석 JSON만 작성하세요. 특히 한국어 띄어쓰기와 어체를 반드시 교정하세요.",
        },
        {
          role: "user",
          content: buildOpenAIUserContent({ prompt, resumePdfBase64, resumePdfImages, resumePdfMimeType }),
        },
      ],
    }),
  });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const raw = await callOpenAIAnalysis({ prompt, resumePdfBase64, resumePdfImages, resumePdfMimeType });
