...
function normalizeQuestion(item, index) {
  let main = cleanText(item.main || item.question || "");
  if (isGenericInterviewQuestion(main)) {
    main = buildSpecificQuestionFromEvidence(item);
  }
  return {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const analysis = normalizeAnalysis(await polishKoreanWithOpenAI(raw));
